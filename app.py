...
def run_pipeline(image, target_country, platform):
    if image is None:
    ...
    creative_pack = generate_creative_pack(strategy_report, target_country, platform)
    
    img_url = creative_pack.get("generated_images", [None])[0] if creative_pack.get("generated_images") else None
    
    yield "✅ 모든 파이프라인 완료!", format_json(product_profile), format_json(context_pack), format_json(strategy_report), format_json(creative_pack), img_url
        ...
        with gr.Column(scale=2):
            with gr.Tabs():
                ...
                with gr.TabItem("4. 크리에이티브 (Execution)"):
                    creative_out = gr.Code(language="json")
                    img_out = gr.Image(label="DALL-E 3 결과물")
                    
    submit_btn.click(
        fn=run_pipeline,
        inputs=[img_input, country_input, platform_input],
