...
from agents.pdf_generator import create_strategy_pdf
...
def run_pipeline(image, target_country, platform, copy_language_mode):
    if image is None:
        yield "이미지를 업로드해주세요.", None, None, None, None, None, None
        return
        
    yield "⏳ 1/5 - Vision Agent: 제품 사진 분석 중...", None, None, None, None, None, None
    product_profile = analyze_product_image(image)
    
    yield "⏳ 2/5 - RAG Router: 검색 쿼리 빌드 및 데이터 수집 중...", format_json(product_profile), None, None, None, None, None
    ...
    yield "⏳ 3/5 - Strategy Agent: 시장 진입 전략 보고서 생성 중...", format_json(product_profile), format_json(context_pack), None, None, None, None
    strategy_report = generate_strategy_report(product_profile, context_pack, target_country)
    
    # Generate PDF
    pdf_path = os.path.join(os.path.dirname(__file__), "strategy_report.pdf")
    create_strategy_pdf(strategy_report, target_country, pdf_path)
    
    yield "⏳ 4/5 - Creative Agent: 마케팅 에셋 및 포스터 생성 중...", format_json(product_profile), format_json(context_pack), format_json(strategy_report), pdf_path, None, None
    ...
    yield "✅ 모든 파이프라인 완료!", format_json(product_profile), format_json(context_pack), format_json(strategy_report), pdf_path, copy_text, img_url
...
with gr.Blocks(title="TrendPilot: K-Beauty Global Agent") as demo:
        ...
        with gr.Column(scale=2):
            with gr.Tabs():
                ...
                with gr.TabItem("3. 전략 보고서 (Strategy)"):
                    strategy_out = gr.Code(language="json", label="JSON 데이터")
                    strategy_pdf_out = gr.File(label="전략 보고서 (PDF)")
    ...
    submit_btn.click(
        ...
        outputs=[status_text, profile_out, context_out, strategy_out, strategy_pdf_out, creative_out, img_out]
    )
